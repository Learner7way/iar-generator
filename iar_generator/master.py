...
def cmd_info(args):
    ...
    print(f"      Размер: {sum(f.stat().st_size for f in project_path.rglob('*') if f.is_file()) / (1024*1024):.2f} MB")
    ...
    if freertos_dir:
        ...
        print(f"\n[*] FreeRTOS:")
        print(f"      FreeRTOSConfig.h не найден")
...
def cmd_check(args):
    ...
    if not freertos_config:
        ...
        print_info(f"FreeRTOSConfig.h найден: {freertos_config.relative_to(project_path)}")
    ...
    if not templates_dir.exists():
        ...
        if missing_templates:
            issues.append(f"Отсутствуют эталонные файлы: {', '.join(missing_templates)}")
...
def main():
    ...
    parser_generate.add_argument('project_path', help='Путь к корневой директории проекта')
    parser_generate.add_argument('project_name', help='Имя проекта')
    parser_generate.add_argument('-o', '--output', help='Выходная директория для IAR файлов')
    ...
    parser_clean.add_argument('-f', '--force', action='store_true', help='Принудительное удаление без подтверждения')
