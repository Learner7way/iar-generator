...
1. ✅ Этап 1: тесты зелёные, coverage на ядре (102 теста, 71%).
2. ✅ Этап 4: в py_generator один генератор IAR (`iar_generator/`).
3. ✅ Этап 5: конфиг читается кодом (`pipeline.ini` + `core/config.py`).
4. ✅ Этап 7: нет хака кодировок (`fix_script_encoding` удалён, эмодзи убраны).
5. ✅ Идеи структуры `core/`+`modules/` перенесены в py_generator (п. 3.1:
   `core/config.py`, `utils/`, `ai/` по образцу rlm_agent).

**Статус (2026-08-17):** все решения перенесены. `C:\sandbox\projects\py_project`
готов к удалению (по отдельному подтверждению пользователя).
