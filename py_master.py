...
HISTORY_DIR = cfg.history_dir  # Папка для хранения истории
TASK_FILE = cfg.task_file  # Файл для сохранения текста задачи
...
def clean_output_files():
    ...
    files_to_clean = [cfg.output_file, cfg.answer_file, cfg.formatted_file]
    cleaned = []
    ...
    for file_path in files_to_clean:
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                f.write("")
            print(f"  • {file_path.name} -> создан пустой файл")
        except Exception as e:
            print(f"  • ОШИБКА при создании {file_path.name}: {e}")
...
def main():
    ...
    if script_add_prompt.exists():
        success1 = run_script(
            str(script_add_prompt), [], 1, "Добавление содержимого promt.md в py_out.md"
        )
    ...
    print(f"Входной файл:    {cfg.output_file}")
    print(f"Выходной файл:   {cfg.answer_file}")
    print(f"Упрощенный:      {cfg.formatted_file}")
    print(f"Файл задачи:     {cfg.task_file}")
    print(f"Папка истории:   {cfg.history_dir}")
    print("=" * 60)
