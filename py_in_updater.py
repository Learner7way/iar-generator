...
def parse_py_in_file(file_path):
    ...
    if 'Содержимое файлов:' in content:
        ...
        for section in sections:
            ...
            if file_path_match:
                ...
                if any(word in section.lower() for word in ['удален', 'removed', 'deleted']):
    ...
    header_section = content.split('Содержимое файлов:')[0] if 'Содержимое файлов:' in content else content
    ...
    for pattern, pattern_type in operation_patterns:
        ...
        for match in matches:
            if pattern_type == 'list_with_status':
                ...
                if any(word in status_lower for word in ['удален', 'removed', 'deleted']):
                    if file_path not in files_to_delete and file_path not in files_to_update:
                        files_to_delete.append(file_path)
                    ...
                    if file_path not in files_to_update:
                        ...
                        if not found:
                            print(f"   [⚠️] File marked as created but not found in content: {file_path}")
                
            elif pattern_type == 'list_simple':
                file_path = match.strip() if isinstance(match, str) else match[0].strip()
                if (file_path not in files_to_update and 
                    file_path not in files_to_delete and
                    not any(file_path in f for f in files_to_update.keys())):
                ...
                if pattern_type == 'delete':
                    if file_path not in files_to_delete and file_path not in files_to_update:
                        files_to_delete.append(file_path)
                        print(f"   [⌫] File marked for deletion (explicit): {file_path}")
                else:  # create/update
                    if file_path not in files_to_update:
                        ...
                        if 'Содержимое файлов:' in content:
                            ...
                            for section in sections:
                                if file_path in section and '```c' in section:
                                    content_match = re.search(r'```c\n(.*?)```', section, re.DOTALL)
                                    if content_match:
                                        files_to_update[file_path] = content_match.group(1).strip()
                                        print(f"   [📄] Found content for listed file: {file_path}")
                                        found = True
                                        break
                        
                        if not found:
                            print(f"   [⚠️] File listed for update but no content found: {file_path}")
    
    # Удаляем дубликаты
    files_to_update = {k: v for k, v in files_to_update.items() if k not in files_to_delete}
...
def git_commit_changes(repo_path, message_prefix, version_info=None):
    ...
    success, out, err = run_git_command(repo_path, ["git", "commit", "-m", commit_message])
    
    if success:
        print(f"   [✓] Commit created: {commit_message}")
        # Получаем хеш коммита
        success, out, err = run_git_command(repo_path, ["git", "rev-parse", "--short", "HEAD"])
...
def print_summary(results, base_dir, start_time, git_commits, version_info=None):
    ...
    if version_info:
        print(f"Version: {version_info.get('current', 'N/A')} → {version_info.get('after', 'N/A')}")
        print("-"*60)
...
def main():
    ...
    # Путь к py_in.txt в текущей директории
    script_dir = Path.cwd()
    py_in_path = script_dir / "py_in.txt"
    ...
    if use_git:
        has_changes = (
            any(status in ['created', 'updated'] for status in results['files'].values()) or
            any(status == 'deleted' for status in results['deleted'].values())
        )
        ...
        if has_changes:
            ...
            success_count = (
                sum(1 for status in results['files'].values() if status in ['created', 'updated']) +
