...
def parse_ewp_file(ewp_file_path, proj_dir):
    ...
    try:
                ...
                for file_elem in group_element.findall('file'):
                    ...
                    if name_elem is not None and name_elem.text:
                        ...
                        try:
                            ...
                            print(f"   [!] Ошибка обработки пути '{file_path_str}': {e}")
        ...
        print(f"\n   Статистика:")
        print(f"      - Файлов в группах firmware/libs/middleware: {len(project_files['structure_only'])}")
...
def get_directory_tree_structure(files, proj_dir):
        ...
        try:
            ...
            try:
                ...
                for i, part in enumerate(file_path.parts):
                    if part.lower() in ['project', 'firmware', 'libs', 'middleware', 'src']:
                        start_index = i
                        break
    ...
    print(f"   [*] Построено структуры: {processed_files} файлов обработано, {skipped_files} пропущено")
    return structure
...
def print_tree(structure, indent="", output_lines=None, max_files=None):
    ...
    for i, dir_name in enumerate(dirs):
        is_last_dir = (i == len(dirs) - 1) and ('_files' not in structure or not structure['_files'])
...
def generate_markdown_content(project_files, proj_dir, ewp_file_name):
    ...
    lines.append(f"- **Файлов в группах firmware/libs/middleware (только структура):** {len(project_files['structure_only'])}")
    lines.append(f"- **Файлов для полного анализа:** {len(project_files['full_content'])}")
    
    # СТРУКТУРА ДИРЕКТОРИЙ
    if project_files['structure_only']:
        lines.append("\n### Структура директорий (firmware/libs/middleware)")
        lines.append("*Эти директории содержат системные/библиотечные файлы, поэтому показывается только структура без содержимого*\n")
        
        structure = get_directory_tree_structure(project_files['structure_only'], proj_dir)
    ...
    if project_files['full_content']:
        ...
        if not text_files:
            ...
            lines.append(f"**Найдено файлов для анализа:** {len(text_files)} (исключено {len(binary_files)} бинарных файлов)\n")
...
def main():
    ...
    # Жестко задаем выходной файл
    output_file_path = script_dir / "py_out.md"
    ...
    markdown_content = generate_markdown_content(project_files, args.proj_dir, ewp_file_path.name)
    ...
    if append_to_file(output_file_path, markdown_content):
        ...
        if project_files['full_content']:
            binary_count = sum(1 for f in project_files['full_content'] if is_binary_file(f))
